...
LOG_MODULE_REGISTER(artnet, LOG_LEVEL_DBG);
#include "private.h"
 ...
 */
artnet_node artnet_new(struct net_if *iface, int verbose) {
  node n;
  int i;
  ...
  /*
  memcpy(&n->state.ip_addr.s4_addr, &iface->config.ip.ipv4->unicast[0].address.in_addr.s4_addr, ARTNET_IP_SIZE);
  memcpy(&n->state.hw_addr, &iface->if_dev->link_addr.addr, ARTNET_MAC_SIZE);
  memcpy(&n->state.bcast_addr.s4_addr, &iface->config.ip.ipv4->unicast[0].address.in_addr.s4_addr, ARTNET_IP_SIZE);
  n->state.bcast_addr.s4_addr[3] = 255;
  */

   n->state.ip_addr = iface->config.ip.ipv4->unicast[0].address.in_addr;
  memcpy(&n->state.hw_addr, &iface->if_dev->link_addr.addr, ARTNET_MAC_SIZE);
  n->state.bcast_addr.s_addr = n->state.ip_addr.s_addr;
  n->state.bcast_addr.s4_addr[3] = 255;
