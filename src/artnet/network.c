...
int artnet_net_start(node n) {
  ...
  if (n->peering.master == TRUE) {
    ...
    servAddr = malloc(sizeof(struct sockaddr));
    memset(servAddr, 0x00, sizeof(struct sockaddr));
    ...
    if (n->state.verbose)
      printf("Binding to %s   -> socket: %d \n", inet_ntop(AF_INET, &servAddr->sin_addr, buf, sizeof(buf)), sock);

     
    // bind sockets
    if (bind(sock, (SA *) &servAddr, sizeof(servAddr)) == -1) {
      printf("@@@@ Failed to bind to socket: errno=%d\n", errno);
...
int artnet_net_close(artnet_socket_t sock) {
  if (zsock_close(sock)) {
    artnet_error("error artnet_close_socket");
    return ARTNET_ENET;
  }
